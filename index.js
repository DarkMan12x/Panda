...
if(!args[0]) return message.channel.send('Please mention someone for band') 
...
return message.channel.send('You cant banned this member') 
...
if(!message.member.hasPermission('BAN_MEMBERS')) return message.channel.send('Sorry you cant perform this command')
    let args = message.content.split(" ").slice(1)
if(!args[0]) return message.channel.send('please mention someone for unabanding')
const reason = args[1] || "With out any reason"
...
client.on('message',async message => {
  if(message.content.startsWith(PREFIX + "vkick")) { 
   if (!message.member.hasPermission("MANAGE_GUILD")) {
      return message.channel.send("You need a role to do this ");
    }
 
    if (!message.mentions.members.first())
      return message.channel.send(
        `Mension someone to kick in the voice!`
      );
 
    let { channel} = message.mentions.members.first().voice;
 
    if (!channel)
      return message.channel.send(`This member is not in voice`);
 
    message.mentions.members.first().voice.kick();
 
    message.channel.send(`Kicked in the voice !`)
  }
})
...
if (!message.member.hasPermission("MANAGE_GUILD")) {
    ...
    if (!message.guild.me.hasPermission("MANAGE_GUILD")) {
      return message.channel.send("I dont have a role to do this");
    } 
    let qawrma = message.mentions.members.first();
    if(!qawrma) return message.reply(`Mention someone !`)
